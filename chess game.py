...
# white piece input
def validate_white_input(user_input):
    try:
...
# black piece input
def validate_black_input(user_input, black_pieces, white_coordinate):
    if user_input == "done":
...
if __name__ == "__main__":
    ...
    while True:
        ...
        is_valid, error_message = validate_white_input(white_input)
        if is_valid:
    ...
    while True:
        ...
        is_valid, error_message = validate_black_input(black_input, black_pieces, white_coordinate)
...
# 1. The user is expected to enter valid formats for all inputs.
# 2. The program board uses the standard 8x8 with valid coordinates (a1 to h8).
# 3. Black pieces cannot overlap in coordinate.
# 4. The pawn's capturing direction assumes the board's "forward" direction is fixed.

#                               lIMITATION
# 1. If the white piece is not "pawn" or "rook," no additional movement rules are applied.
